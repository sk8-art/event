...
<div class="festivals-page">
    ...
    <div class="calendar-section">
        ...
        <form id="dateForm" action="{{ route('festivals') }}" method="GET" style="display: none;">
    ...
    <!-- Список фестивалей - карточки как на главной -->
    <div class="events-grid" id="eventsContainer">
        @forelse($festivals as $festival)
            <div class="event-card" onclick="window.location='{{ route('events.show', $festival) }}'">
                <!-- Фоновая картинка -->
                ...
                <div class="event-content">
                    <h3 class="event-title">{{ $festival->title }}</h3>
                    
                    <!-- Дата, время и место -->
                    <div class="event-datetime">
                        <span class="datetime-item">{{ $festival->russian_date }}</span>
                        <span class="datetime-item">{{ $festival->formatted_time }}</span>
                        <span class="separator">•</span>
                        <span class="datetime-item">{{ $festival->location }}</span>
                    </div>
                    ...
                    <div class="price-buy">
                        ...
                        <div class="event-meta">
                            <span class="event-price">
                                @if($festival->price > 0)
                                    от {{ number_format($festival->price, 0, ',', ' ') }} ₽
                                @else
                                    Бесплатно
                                @endif
                            </span>
                        ...
                        <!-- Кнопка Купить билет -->
                        <a href="{{ route('events.show', $festival) }}" class="event-button" onclick="event.stopPropagation();">
                            Купить билет
                        </a>
                    </div>
                </div>
        ...
        @empty
            <div class="no-results">
                <div class="no-results-content">
                    ...
                    <h3>Фестивали не найдены</h3>
                    <p>Попробуйте изменить параметры поиска</p>
                    <a href="{{ route('festivals') }}" class="btn-reset-large">
                        <span>↺</span>
                        Сбросить фильтры
                    </a>
                </div>
...
/* Стили для карточек как на главной */
.events-grid {
    display: grid;
